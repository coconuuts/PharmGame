...
public class OpenNPCInventory : MonoBehaviour, IInteractable
{
    ...
    public string InteractionPrompt => interactionPrompt;
        ...
        void Awake()
        {
            ...
            if (inventoryComponent == null)
            {
                ...
                if (inventoryGameObject != null)
                {
                    ...
                    inventoryComponent = inventoryGameObject.GetComponent<InventoryClass>();
                ...
                else
                {
                    ...
                    // Optional: Handle the case where the inventory GameObject is not found.
                }
            }
        }
        ...
        void Start()
        {
            // Get the UI Root reference from the UIManager singleton
            if (NPCUIManager.Instance != null)
            {
                inventoryUIRoot = NPCUIManager.Instance.inventoryUIRoot;

                if (inventoryUIRoot != null)
                {
                    Debug.Log("Successfully got Inventory UI Root reference from UIManager.");
                }
                else
                {
                    Debug.LogError("UIManager exists, but the Inventory UI Root reference is NOT assigned in the UIManager inspector!");
                }
            }
            else
            {
                Debug.LogError("UIManager instance not found in the scene. Make sure it exists and has the UIManager script.");
            }
        }

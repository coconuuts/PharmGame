// --- START OF FILE BasicWaitingForDeliverySimSO.cs ---
...
namespace Game.NPC.BasicStates // Place Basic States in their own namespace
{
    ...
    /// for delivery (corresponds to active WaitingForDelivery).
    /// Relies on the BasicNpcStateManager's timeout logic for progression.
    /// Operates directly on TiNpcData.
    /// Corresponds to BasicState.BasicWaitingForDeliverySim.
    /// </summary>
    [CreateAssetMenu(fileName = "BasicWaitingForDeliverySimState", menuName = "NPC/Basic States/Basic Waiting For Delivery Sim", order = 8)] // Order after BasicWaitingAtPrescriptionSpot
    public class BasicWaitingForDeliverySimSO : BasicNpcStateSO
    {
        // Set the HandledBasicState to the new enum value
        public override System.Enum HandledBasicState => BasicState.BasicWaitingForDeliverySim;
        ...
        // [Header("Prescription Delivery Waiting Timeout")]
        // [SerializeField] private Vector2 deliverySimInactiveTimeout = new Vector2(20f, 40f); // Example range
        // public override float MinInactiveTimeout => deliverySimInactiveTimeout.x;
        // public override float MaxInactiveTimeout => deliverySimInactiveTimeout.y;
        ...
        public override void OnEnter(TiNpcData data, BasicNpcStateManager manager)
        {
            ...
            Debug.Log($"SIM {data.Id}: BasicWaitingForDeliverySimState OnEnter. Will remain frozen at claim spot until timeout ({data.simulatedStateTimer:F2}s initial).", data.NpcGameObject);

            // Note: Position and rotation should already be set to the claim spot's position/rotation
            // by the previous state (BasicWaitingAtPrescriptionSpotSO or BasicLookToPrescriptionStateSO) upon arrival.
        }
...
// --- END OF FILE BasicWaitingForDeliverySimSO.cs ---
...
namespace Systems.UI
{
    ...
    public class UIManager : MonoBehaviour
    {
        ...
        /// <summary>
        /// Handles the visibility of UI elements based on game state changes.
        ...
        private void HandleGameStateChanged(MenuManager.GameState newState, MenuManager.GameState oldState, InteractionResponse response) // ADDED PARAM
        {
            ...
            switch (oldState)
            {
                case MenuManager.GameState.InInventory:
                    ...
                    if (MenuManager.Instance != null && MenuManager.Instance.CurrentActiveUIRoot != null)
                    {
                        ...
                        if (MenuManager.Instance.CurrentOpenInventoryComponent != null)
                        {
                            ...
                            Debug.Log($"UIManager: Cleared hover highlights for Inventory.");
                        }
                    }
                ...
                case MenuManager.GameState.InMinigame:
                    // Deactivate the Minigame UI root that was active (accessed via MenuManager getter)
                    if (MenuManager.Instance != null && MenuManager.Instance.CurrentActiveUIRoot != null)
                    {
                        MenuManager.Instance.CurrentActiveUIRoot.SetActive(false);
                        Debug.Log($"UIManager: Deactivated Minigame UI Root.");
                    }
                    else Debug.LogWarning($"UIManager: Cannot deactivate Minigame UI when leaving {oldState} - no stored root or manager instance.");
                    break;
                case MenuManager.GameState.InCrafting:
                    ...
                    if (MenuManager.Instance != null && MenuManager.Instance.CurrentCraftingStation != null)
                    {
                        ...
                        Debug.Log($"UIManager: Told CraftingStation to close UI.");
                        // Call the MenuManager helper to clear hover highlights for crafting inventories
                        // Need access to the specific inventories from the CraftingStation
                        CraftingStation currentCraftingStation = MenuManager.Instance.CurrentCraftingStation;
                        if (currentCraftingStation != null) // Added safety check
                        {
                            ...
                            Debug.Log($"UIManager: Cleared hover highlights for Crafting Inventories.");
                        }
                ...
                case MenuManager.GameState.InPauseMenu:
                    ...
                    break;
                    // Playing state doesn't have specific UI to deactivate on exit (Player/Toolbar handled below)
            }
            // -------------------------------------------
            ...
            switch (newState)
            {
                ...
                case MenuManager.GameState.InInventory:
                    ...
                    if (response is OpenInventoryResponse openInventoryResponse)
                    {
                        if (openInventoryResponse.InventoryUIRoot != null)
                        {
                            ...
                            // Note: Initial highlight/setup for the Inventory UI itself
                            // should ideally be handled by the InventoryVisualizer/InventorySlotUI scripts
                            // when the UI GameObject becomes active and they get their data.
                            // InventorySelector handles the *selection* highlight on the toolbar.
                        }
                ...
                case MenuManager.GameState.InComputer:
                    ...
                    if (response is EnterComputerResponse enterComputerResponse)
                    {
                        if (enterComputerResponse.ComputerUIRoot != null)
                        {
                            ...
                            // Note: Computer UI initialization should be handled by a script on that UI root
                            // or via a StateAction SO if complex setup is needed.
                        }
                ...
                case MenuManager.GameState.InMinigame:
                    ...
                    // Dynamic UI: Activate the Minigame UI root directly from the response
                    if (response is StartMinigameResponse startMinigameResponse)
                    {
                        if (startMinigameResponse.MinigameUIRoot != null)
                        {
                            startMinigameResponse.MinigameUIRoot.SetActive(true);
                            Debug.Log($"UIManager: Activated Minigame UI Root from Response.");
                            // Note: Minigame setup (spawning barcodes) is handled by MinigameManager.StartMinigame,
                            // which is called by a StateAction SO, not directly here.
                        }
                        else Debug.LogWarning("UIManager: StartMinigameResponse did not contain a valid UI Root to activate.");
                    }
                    else Debug.LogWarning("UIManager: Entered InMinigame state, but response was not StartMinigameResponse.");
                    break;
                ...
                case MenuManager.GameState.InCrafting:
                    ...
                    if (response is OpenCraftingResponse openCraftingResponse)
                    {
                        if (openCraftingResponse.CraftingStationComponent != null)
                        {
                            ...
                            // Note: Crafting UI initialization should be handled by the CraftingStation itself
                            // when its OpenCraftingUI method is called.
                        }
